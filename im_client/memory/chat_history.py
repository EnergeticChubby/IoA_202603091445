...
from llms.utils import count_message_tokens, count_string_tokens
...
'''
    ...
    def to_string(self, add_sender_prefix: bool = False) -> str:
        if add_sender_prefix:
            return "\n".join(
                [
                    f"[{message.sender}]: {message.content}" if message.sender != "" else message.content
    ...
    async def to_messages(
        ...
        messages = [message.to_openai_message() for message in self.messages[start_index:]]
        ...
        if self.has_summary:
            ...
            prompt = []
            trimmed_history = add_history_upto_token_limit(prompt, messages, max_send_token, model)
            if trimmed_history:
                new_summary_msg, _ = await self.trim_messages(list(prompt), model, messages)
    ...
    async def trim_messages(
        ...
        new_messages_not_in_chain = [msg for msg in history if msg not in current_message_chain]

        if not new_messages_not_in_chain:
            return self.summary_message(), []

        new_summary_message = await self.update_running_summary(new_events=new_messages_not_in_chain, model=model)
    ...
    async def update_running_summary(
        ...
        prompt_template_length = len(Template(self.SUMMARIZATION_PROMPT).safe_substitute(summary="", new_events=""))
        ...
        for event in new_events:
            ...
            if batch_tlength + event_tlength > max_input_tokens - prompt_template_length - summary_tlength:
    ...
    async def _update_summary_with_batch(
        ...
        prompt = Template(self.SUMMARIZATION_PROMPT).safe_substitute(summary=self.summary, new_events=new_events_batch)
...
def add_history_upto_token_limit(prompt: List[dict], history: List[dict], t_limit: int, model: str) -> List[LLMResult]:
    limit_reached = False
